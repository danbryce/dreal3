...
void plan_heuristic::inform(Enode * e) {
    if (m_atoms.find(e) != m_atoms.end()) return;
    m_atoms.insert(e);
...
int plan_heuristic::getChoiceIndex(Enode * e) {
    ...
    if (i == choice_indices.end()) {
        ...
        for (auto const & v : vars) {
            ...
            ss << v;
            string var = ss.str();
            if (var.find("duract_") != string::npos || var.find("act_") != string::npos) {
                int spos = var.find_first_of("_") + 1;
                int epos = var.find_last_of("_") - 1;
                string proc = var.substr(spos, epos - spos).c_str();
...
void plan_heuristic::pushTrailOnStack() {
    DREAL_LOG_INFO << "plan_heuristic::pushTrailOnStack() lastTrailEnd = " << lastTrailEnd
                   << " trail->size() = " << trail->size();
    if ((unsigned int)trail_lim->size() >
        m_stack_lim.size())  // track start of levels after the first level
...
void plan_heuristic::completeSuggestionsForTrail() {
    for (unsigned int i = 0; i < m_decision_stack.size(); i++) {
        ...
        if (decision->first != NULL) {
            ...
            m_suggestions.push_back(
                new std::pair<Enode *, bool>(decision->first, decision->second->back()));
        }
    }
    ...
    //   //     DREAL_LOG_INFO << "plan_heuristic::completeSuggestionsForTrail() suggesting = " <<
    //   p.second;
...
bool plan_heuristic::expand_path() {
    ...
    if (m_decision_stack.size() == 0 && !first_path) return false;

    first_path = false;

    int steps_to_add =
        (num_choices_per_happening * m_depth) - static_cast<int>(m_decision_stack.size());
    ...
    for (int i = 0; i < steps_to_add; i++) {
        int time =
            m_depth - ((static_cast<int>(m_decision_stack.size())) / num_choices_per_happening);
        ...
        //        dec->insert(dec->begin(), m_goal_modes[autom]->begin(),
        //        m_goal_modes[autom]->end());
	 ...
	 if(current_enode != NULL){
	  ...
	  if (e->first == current_enode) {
        ...
        // current_decision->insert(current_decision->begin(), current_decision_copy.begin(),
        // current_decision_copy.end());
        ...
        m_decision_stack.push_back(
            new pair<Enode *, vector<bool> *>(current_enode, current_decision));
...
bool plan_heuristic::getSuggestions() {
    ...
    if (trail->size() > lastTrailEnd) pushTrailOnStack();
    ...
    if (m_decision_stack.size() == 0) return false;
