...
mcts_node::~mcts_node() {
  ...
  // remove parent w/o any children
   if (auto par = m_parent.lock()) {
      ...
      // auto it = std::find(parent_children->begin(), parent_children->end(), shared_ptr<mcts_node>(this));
      //   if (it != parent_children->end()) {
      // 	  DREAL_LOG_INFO << "deleting " << m_parent->id() << " pointer to " << m_id;
      // 	  parent_children->erase(it);
      //   }
      // 	DREAL_LOG_INFO << m_parent->id() << " has " << parent_children->size() << " children";
	  ...
	  if(auto gpar = par->parent().lock()){
	    ...
	    auto it = std::find(grandparent_children->begin(), grandparent_children->end(), par);
  ...
  shared_ptr<mcts_node> selected;
	...
	child->set_score(score);
         DREAL_LOG_INFO << "mcts_node::select(" << m_id
			...
			<< " " << (child->value() / child->visits()) << " " << (sqrt(log(m_visits) / child->visits()));
